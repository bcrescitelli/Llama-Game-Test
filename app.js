<script>
/* ===== Short utilities & state ===== */
const STORAGE_KEY = "llamalog_state_v6";
...
const UI = {
  show(id){ $$("section").forEach(s=>s.classList.add("hidden")); $("#"+id)?.classList.remove("hidden");
    if(id==="managerDashboard") Screens.renderManager(); if(id==="repDashboard") Screens.renderRep(); },
  ...
  openChat(){ $("#chatModal").classList.add("show"); Screens.renderChat(); markChatRead(); }, closeChat(){ $("#chatModal").classList.remove("show"); }
};
...
function confettiBurst(n=40){
  ...
  for(let i=0;i<n;i++){
    ...
    s.style.cssText=`position:fixed;top:10px;right:10px;width:8px;height:8px;background:hsl(${Math.random()*360}deg 90% 60%);border-radius:2px;transform:translate(${(Math.random()*-160)}px,${(Math.random()*60)}px) rotate(${Math.random()*360}deg);opacity:0.95;z-index:9999;`;
    frag.appendChild(s);
    const dx=(Math.random()*2-1)*200, dy=200+Math.random()*300, rot=360+Math.random()*360, t=800+Math.random()*800;
    s.animate([{transform:s.style.transform,opacity:0.95},{transform:`translate(${dx}px,${dy}px) rotate(${rot}deg)`,opacity:0}],{duration:t,easing:"cubic-bezier(.2,.8,.2,1)"}).onfinish=()=>s.remove();
  }
  document.body.appendChild(frag);
}

/* ===== Domain: teams/members ===== */
...
function townTick(team){
  ...
  const a=team.town.active; a.activeMs = (a.activeMs||0) + 1000; // tick each second
  if(a.activeMs >= a.durationMs){
    ...
    if(b.rareDrop && Math.random()<0.25){
      ...
      if(mem){
        if(CATALOG.accessories.find(i=>i.key===b.rareDrop)) mem.owned[b.rareDrop]=true;
        if(CATALOG.hats.find(i=>i.key===b.rareDrop)) mem.owned[b.rareDrop]=true;
      }
    }
...
const gateAny = (team, arr)=> arr.some(k=>gate(team,k));
...
const Screens={
  renderManager(){
    ...
    // team town (manager sees same panel as reps)
    renderTownPanel(t);
    save(s);
    updateChatBadges();
  },
  ...
  renderShop(tab="hats"){
    ...
    const addCard=(item,type)=>{
      ...
      const media = isFood? `<div class="text-4xl">${item.emoji}</div>` : isBg? `<div class="w-full h-full" style="background:${item.thumbCSS};"></div>` : `<img src="${item.thumb}" onerror="this.src='${item.image}'" class="w-full h-full object-contain" alt="${item.name}">`;
      div.innerHTML = `<div class="aspect-video bg-white/5 rounded-xl mb-2 flex items-center justify-center overflow-hidden">${media}</div>
        ...
        <div class="grid grid-cols-2 gap-2">
          ${
            isFood ? `<button class="bg-indigo-500 hover:bg-indigo-600 px-2 py-1.5 rounded text-sm" onclick="Handlers.buyFood('${item.key}')">Buy</button>
    ...
    // if current tab locked, jump to any allowed
    if(!tabAllowed[tab]) tab = Object.keys(tabAllowed).find(k=>tabAllowed[k]) || "accessories";
    window.currentShopTab = tab;
    $$("#shopModal .tab-btn").forEach(b=>{
      const k=b.dataset.tab; b.disabled=!tabAllowed[k]; b.title = tabAllowed[k]?"": "Build to unlock";
      b.classList.toggle("opacity-50", !tabAllowed[k]);
    });
    const source = tab==="food"? Object.entries(CATALOG.food).map(([key,v])=>({...v,key}))
...
const Handlers={
  ...
  applyBackground(k){ const s=load(), t=s.teams[session.teamCode], m=t.members[session.email]; m.equipped.background=k; save(s); Toast("Background applied","success"); Screens.renderRep(); Screens.renderShop(window.currentShopTab||"hats"); },
  ...
  adjustProgress(i,delta){ const s=load(), t=s.teams[session.teamCode], m=t.members[session.email]; const d=todayKey(), a=t.activities[i]; if(!a?.target) return;
    // suspicious: >5 clicks in 20s OR interval <350ms
  ...
  checkPath(t,m){ const d=todayKey(); const all=(t.activities||[]).slice(0,4).every((a,i)=>a.target? (m.progress[d][i].count>=a.target): !!m.progress[d][i].done);
    if(all){ m.coins+=8; addXP(m,6); Toast("Path complete! +8 coins","success"); if(gate(t,"perk:celebration")) confettiBurst(80); } },
  ...
  submitMood(v){ const s=load(), t=s.teams[session.teamCode], m=t.members[session.email]; const now=new Date(); m.checkins[todayKey()]={mood:v,at:now.getTime()};
    if(now.getHours()<10) Badges.unlock(m,"streak_3"); m.llama.happiness=Math.min(100,m.llama.happiness+Math.max(0,v-2)); save(s); Toast("Mood saved","success"); UI.closeMood(); Screens.renderRep(); Screens.renderManager(); },
  ...
  sendChat(e){ e.preventDefault(); const input=$("#chatInput"); const text=input.value.trim(); if(!text) return; const s=load(), t=s.teams[session.teamCode];
    if(!gate(t,"chat")) return Toast("Build Networking Center to unlock chat","warn");
  ...
  // Gifts
  repGift(e){ e.preventDefault(); const s=load(), t=s.teams[session.teamCode], m=t.members[session.email]; if(!gate(t,"gifting")) return Toast("Build Networking Center to unlock gifting","warn");
  ...
  managerGift(e){ e.preventDefault(); const s=load(), t=s.teams[session.teamCode]; if(!gate(t,"gifting")) return Toast("Build Networking Center to unlock gifting","warn");
...
function renderGiftingSelectors(t,m){
  const recSel=$("#repGiftRecipient"); recSel.innerHTML="";
  Object.values(t.members).forEach(x=>{ if(x.email===m.email) return; const o=document.createElement("option"); o.value=x.email; o.textContent=x.firstName||x.email; recSel.appendChild(o); });
  const itemSel=$("#repGiftItem"); itemSel.innerHTML=""; Object.entries(m.inventory).forEach(([k,q])=>{ if(q>0){ const o=document.createElement("option"); o.value=k; o.textContent=`${CATALOG.food[k].emoji} ${CATALOG.food[k].label} (You have ${q})`; itemSel.appendChild(o); }});
  // Manager gifting options
...
(function seed(){
  ...
  const code=uid(10); s.teams[code]={
    teamName:"Demo Team", managerEmail:"manager@example.com", activities:[
      {label:"Log calls",target:3},{label:"Send follow ups",target:5},{label:"Update pipeline",target:null},{label:"Prep opportunities",target:null}
